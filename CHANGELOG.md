...
- **Package renamed** from `@trymightyai/citadel-guard-openclaw` to `@mightyai/citadel-guard-openclaw`

### Added

- **SHA256 checksum verification** for Go binary downloads during postinstall
- **`CITADEL_REQUIRE_CHECKSUM`** env var for strict mode (fails if checksums unavailable)
- Graceful degradation when `checksums.txt` is not yet published in a release
